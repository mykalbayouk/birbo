...
export default function Navbar({ logged, setLogged }: { logged: boolean; setLogged: () => void } ) {
  return (
    <header className="w-full flex justify-between items-center bg-[#fdf7e1] p-4 rounded shadow-md">
      ...
      {logged ? (
        <Link href="/">
          <button onClick = {setLogged} className="bg-[#fdd28e] text-sm text-gray-800 px-4 py-2 rounded-full shadow">
            Logout
          </button>
        </Link>
      ) :
